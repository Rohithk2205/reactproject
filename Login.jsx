...
function Login({ baseUri }) {
    ...
    const signIn = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/chatify/login', {
                username,
                password,
            });
            ...
            setTimeout(() => {
                ...
                navigate('/home', {
                    state: { userId: response.data.userId, userName: username, active: response.data.isactive },
                });
            }, 1000);
        } catch (error) {
            setIsLoading(false);
            ...
            <div id="content">
                <div className="border">
                    ...
                    <form onSubmit={handleSubmit}>
                        <label
                            className="label"
                            style={{ fontSize: '24px', fontStyle: 'italic', color: '#666666' }}>
                            Username:
                        </label>
                        ...
                        />
                        {isUserError && <div className="error" style={{ fontSize: '18px', color: '#FF0000' }}>Username is required</div>}
                        <br />
                        <br />
                        <label
                            className="label"
                        ...
                        <input
                            ...
                            style={{ fontSize: '20px', padding: '10px', borderRadius: '5px', border: '1px solid #CCCCCC' }}
                        />
                        {isPassError && <div className="error" style={{ fontSize: '18px', color: '#FF0000' }}>Password is required</div>}
                        <br />
                        <br />
                        {error && <div className="error" style={{ fontSize: '18px', color: '#FF0000' }}>{error}</div>}
                        <br />
                        {successMessage && <div style={{ color: 'green', fontSize: '18px' }}>Login successful! Redirecting...</div>}
